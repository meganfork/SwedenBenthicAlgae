...
3. Stats on chla data, including polynomial fits for DOC, light, and TDN
4. Water chemistry
...
Figure 3: chla vs. light transmission for C, N, and NP treatments with conceptual overlay for habitat type
Figure S1. Map
...
##### Fitting polynomial curve to chl-a vs. DOC
chla.poly<-with(subset(lake.data.long, Depth=="1" & Treatment == "C"),lm(chla_ug_cm2~poly(DOC_mgL_chem.mean,2,raw=T)))
summary(chla.poly)

butt<-subset(lake.data.long, Depth=="1" & Treatment == "C")[,c("chla_ug_cm2","DOC_mgL_chem.mean")]
x<-butt$DOC_mgL_chem.mean
y<-butt$chla_ug_cm2
...
chla.light.poly<-with(subset(light.merge, Treatment == "C" & param == "DOC_mgL"),lm(chla.mean~poly(prop.transmitted,2,raw=T)))
summary(chla.light.poly)

fart<-subset(light.merge, Treatment == "C" & param == "DOC_mgL")[,c("chla.mean","prop.transmitted")]
x<-fart$prop.transmitted
y<-fart$chla.mean
...
# chla.light polynomial fit:
#                                    Estimate Std. Error t value Pr(>|t|)  
#(Intercept)                            2.352      1.424   1.652   0.1193  
#poly(prop.transmitted, 2, raw = T)1   11.448      5.764   1.986   0.0656 .
#poly(prop.transmitted, 2, raw = T)2  -14.139      5.556  -2.545   0.0224 *

#Residual standard error: 1.522 on 15 degrees of freedom
#Multiple R-squared:  0.3885,	Adjusted R-squared:  0.307 
#F-statistic: 4.766 on 2 and 15 DF,  p-value: 0.02499

# chla peak predicted @ 40.4% of incident light transmitted

##### Fitting polynomial curve to chl-a vs. TDN
chla.N.poly<-with(subset(lake.data.long, Depth=="1" & Treatment == "C"),lm(chla_ug_cm2~poly(TDN_ugL_chem.mean,2,raw=T)))
summary(chla.N.poly)

toot<-subset(lake.data.long, Depth=="1" & Treatment == "C")[,c("chla_ug_cm2","TDN_ugL_chem.mean")]
x<-toot$TDN_ugL_chem.mean
y<-toot$chla_ug_cm2
...
text(0,5,"p = 0.025",adj=1)
text(0,4.5,expression(paste("R"^2," = 0.31")),adj=1)
...
arrows(0.404, 1.1, 0.404, 0.9, length = 0.06,angle = 30, code = 2)
text(0.404, 1.15,"inflection point in controls",adj=c(0,0),cex=0.7)
...
with(subset(light.merge, Treatment == "C" & region == "boreal"),plot(chla.mean~prop.transmitted,ylim=c(0,26),xlim=rev(c(0,1)),axes=F,xlab = "Proportion of incident light transmitted to deployment",ylab=expression(paste("Benthic chl-",italic("a")," (",mu,"g cm"^-2,")")),pch=16,cex.lab=0.9))
with(subset(light.merge, Treatment == "C" & region == "arctic"),points(chla.mean~prop.transmitted,pch=17))
with(subset(light.merge, Treatment == "C"),arrows(prop.transmitted,chla.mean-chla.sd/(chla.count^0.5),prop.transmitted,chla.mean+chla.sd/(chla.count^0.5),length = 0.05,angle = 90, code = 3))
...
text(0.3,15,"Epilithic algae",adj=c(0,0),cex=0.8,col="#5c5c5c")
arrows(0.25,14,0.37,3,length = 0.08,angle = 30, code = 2,col="#5c5c5c")

text(1,25,"Potential responses \nof epipelic algae under \ndifferent nutrient \navailabilities",adj=c(0,1),cex=0.8,col="#006967")
