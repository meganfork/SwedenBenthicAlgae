...
3. Stats on chla data, including depth effects and model selection for DOC, light, and TDN gradients
4. Water chemistry
...
Figure S4. absorbance vs. DOC
Figure S5. Within-lake effects of depth on chl-a
Figure S6. chla vs. light transmission for C, N, and NP treatments
...
```{r 3. Stats on chla data, including depth effects and model fits for DOC, light, and TDN gradients}
...
## All in one figure - not in paper
...
# text(c(b,b+3,b+6,b+9),rep(0.4,times=12),c("a","a","ab","ab","bde","ad","ad","de","cde","e","de","abc"),cex=0.7) # comparison among all lakes
text(c(b,b+3,b+6,b+9),rep(0.4,times=12),c("a","a","b","a","a","a","a","a","a","a","ab","b"),cex=0.7) # within lake comparisons
legend("topleft",c("0.1 m","0.5 m","1 m"),pch=15,col = c("gray90","gray55","gray25"),cex=0.8,bty='n')
# dev.off()


## Four panels - Figure S5

setwd("D:/UmeaResearch/ForkWallenbergLakes")

 png("Figures/MAnuscript/controls.depths.withinLake.png",height=3,width=4.5,units='in',res=300)
par(mfcol=c(2,2),
    mar = c(0.5,0.7,0.5,0.5),
    mgp=c(1,0.2,0),
    oma=c(0,3,0,0))
BD06.depth.control.boxplot <- boxplot(chla_ug_cm2~Depth, data=subset(lake.data.long,SampleLoc=="BD06"&Treatment=="C"), at = b, yaxt='n',xaxt='n',col = c("gray90","gray55","gray25"),ylim=c(0.1,7.1),cex.axis=0.8,cex.lab=1,outcex=0.8,ylab="")
axis(2,tck=0.02,cex.axis=0.8)
text(b, rep(0.5,times=3), c("a", "a", "b"))
text(0.9,6.9,"BD06")
legend("topleft",c("0.1 m","0.5 m","1 m"),pch=15,col = c("gray90","gray55","gray25"),cex=0.8,bty='n')

Almb.depth.control.boxplot <- boxplot(chla_ug_cm2~Depth, data=subset(lake.data.long,SampleLoc=="Almb"&Treatment=="C"), at = b, yaxt='n',xaxt='n',col = c("gray90","gray55","gray25"),ylim=c(0.1,7.1),cex.axis=0.8,cex.lab=1,outcex=0.8,ylab=expression(paste("Chl-",italic("a")," (",mu,"g cm"^-2,")")))
axis(2,tck=0.02,cex.axis=0.8)
text(b, rep(0.5,times=3), c("a", "a", "a"))
text(0.9,6.9,"Almb")

mtext(expression(paste("Chl-",italic("a")," (",mu,"g cm"^-2,")")), side =2, line = 1.5, at = 7)

Nast.depth.control.boxplot <- boxplot(chla_ug_cm2~Depth, data=subset(lake.data.long,SampleLoc=="Nast"&Treatment=="C"), at = b, yaxt='n',xaxt='n',col = c("gray90","gray55","gray25"),ylim=c(0.1,7.1),cex.axis=0.8,cex.lab=1,outcex=0.8,ylab="")
axis(2,tck=0.02,cex.axis=0.8)
text(b, rep(0.5,times=3), c("a", "a", "a"))
text(0.9,6.9,"Nast")

Bjorn.depth.control.boxplot <- boxplot(chla_ug_cm2~Depth, data=subset(lake.data.long,SampleLoc=="Bjorn"&Treatment=="C"), at = b, yaxt='n',xaxt='n',col = c("gray90","gray55","gray25"),ylim=c(0.1,7.1),cex.axis=0.8,cex.lab=1,outcex=0.8,ylab="")
axis(2,tck=0.02,cex.axis=0.8)
text(b, rep(0.5,times=3), c("a", "ab", "b"))
text(0.9,6.9,"Bjorn")
...
###############################################
# Plot of a440 vs. measured DOC concentration # - FIGURE S4
###############################################
...
```{r Figure 1: 3 panels for a) chl-a vs. DOC, b) chl-a vs. N and c) chl-a vs. light }
setwd("D:/UmeaResearch/ForkWallenbergLakes")
...
# with(subset(lake.data.wide, Depth!="1" & Treatment == "C" & param == "DOC_mgL"),arrows(chem.mean,chla.mean-(chla.sd/(chla.count^0.5)),chem.mean,chla.mean+(chla.sd/(chla.count^0.5)),length = 0.05,angle = 90, code = 3,col="#59595980")) # This would plot the points for deployments other than 1 m depth
...
text(0.8*max(subset(lake.data.wide, param == "DOC_mgL")$chem.mean),0.8,"p = 0.002",adj=0)
text(0.8*max(subset(lake.data.wide, param == "DOC_mgL")$chem.mean),0.3,expression(paste("R"^2," = 0.28")),adj=0)
text(2,7.75,"A",adj=0,cex=1.5)
mtext(expression(paste("Chl-",italic("a")," (",mu,"g cm"^-2,")")),side=2,line=1.2,cex=0.9)

# panel B:
# to convert to micromolar units for TDN, we'll pull the TDN subset out and divide to convert units before plotting:
TDN.umolar <- subset(lake.data.wide, param == "TDN_ugL") 
TDN.umolar$TDN_umolL <- TDN.umolar$chem.mean/14
with(subset(TDN.umolar, Depth== 1 & Treatment == "C" & region == "boreal"), plot(chla.mean~TDN_umolL, xlim=c((70/14),(330/14)), ylim=c(0,8), axes=F, xlab=expression(paste("[TDN] (",mu,"mol L"^-1,")")),ylab="",pch=16,cex.lab=1.2))
with(subset(TDN.umolar, Depth==1 & Treatment == "C" & region == "arctic" & param == "TDN_ugL"),points(chla.mean~TDN_umolL,pch=17))
# with(subset(TDN.umolar, Depth!=1 & Treatment == "C" & region == "boreal" & param == "TDN_ugL"),points(chla.mean~TDN_umolL,pch=16,col="#59595980"))
# with(subset(TDN.umolar, Depth!=1 & Treatment == "C" & region == "arctic" & param == "TDN_ugL"),points(chla.mean~TDN_umolL,pch=17,col="#59595980"))
with(subset(TDN.umolar, Depth==1 &Treatment == "C" & param == "TDN_ugL"),arrows(TDN_umolL,chla.mean-(chla.sd/(chla.count^0.5)),TDN_umolL,chla.mean+(chla.sd/(chla.count^0.5)),length = 0.05,angle = 90, code = 3))
# with(subset(TDN.umolar,Depth!=1 &Treatment == "C" & param == "TDN_ugL"),arrows(TDN_umolL,chla.mean-(chla.sd/(chla.count^0.5)),TDN_umolL,chla.mean+(chla.sd/(chla.count^0.5)),length = 0.05,angle = 90, code = 3,col="#59595980")) # This would plot the points for deployments other than 1 m depth
points((chla.N.pred$x/14),chla.N.pred$y,type="l",col="blue")
axis(1,tck=0.02,cex.axis=0.9)
axis(2,tck=0.02,labels=F)
text(0.85*max(TDN.umolar$TDN_umolL),0.8,"p < 0.0001",adj=0)
text(0.85*max(TDN.umolar$TDN_umolL),0.3,expression(paste("R"^2," = 0.32")),adj=0)
box()
text(70/14,7.75,"B",adj=0,cex=1.5)

# panel C: chla vs. light transmission for all 18 deployments
...
text(0.25,0.8,"p < 0.0001",adj=0)
text(0.25,0.3,expression(paste("R"^2," = 0.27")),adj=0)
text(1,7.75,"C",adj=0,cex=1.5)
