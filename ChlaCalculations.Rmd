...
# Plot this relationship to use as a supplementary figure in the manuscript
...
```{r Load list of samples and abs spectra files, then calculate chl-a ORTRASK SUBSTRATE EXPERIMENT}

#Load list of samples from JULY 2019 ONLY
Ortrask.sample.list<-read.csv("Ortrask_chla_filesID.csv",header=T)

setwd("chlaCSVs/OrtraskSubstrateChla")
...
#read in the individual sample spectra
blanks<-lapply(files.blanks,read.csv,header=T)
names(blanks)<-files.blanks
acid<-lapply(files.acidified,read.csv,header=T)
names(acid)<-files.acidified
unacid<-lapply(files.unacidified,read.csv,header=T)
names(unacid)<-files.unacidified
...
#Vector to store chla calculations
Ortrask.chla.ug.cm2<-NULL
for (i in 1:nrow(Ortrask.sample.list)){
    Ortrask.chla.ug.cm2[i]<-26.7*
    ((as.numeric(as.character(unacid[[which(names(unacid)==paste0(Ortrask.sample.list[["filename"]][i],".csv"))]][which(as.numeric(as.character(unacid[[which(names(unacid)==paste0(Ortrask.sample.list[["filename"]][i],".csv"))]][,1]))==665),10]))-as.numeric(as.character(unacid[[which(names(unacid)==paste0(Ortrask.sample.list[["filename"]][i],".csv"))]][which(as.numeric(as.character(unacid[[which(names(unacid)==paste0(Ortrask.sample.list[["filename"]][i],".csv"))]][,1]))==700),10])))
                    -(as.numeric(as.character(acid[[which(names(acid)==paste0(Ortrask.sample.list[["filename"]][i],"acid.csv"))]][which(as.numeric(as.character(acid[[which(names(acid)==paste0(Ortrask.sample.list[["filename"]][i],"acid.csv"))]][,1]))==664),10]))-as.numeric(as.character(acid[[which(names(acid)==paste0(Ortrask.sample.list[["filename"]][i],"acid.csv"))]][which(as.numeric(as.character(acid[[which(names(acid)==paste0(Ortrask.sample.list[["filename"]][i],"acid.csv"))]][,1]))==700),10]))))*Vext/t.area
}

Ortrask.sub.trt<-paste0(Ortrask.sample.list[["Substrate"]],"_",Ortrask.sample.list[["Treatment"]]) 
Ortrask.chla.data<-cbind(Ortrask.sample.list,Ortrask.sub.trt,Ortrask.chla.ug.cm2)
colnames(Ortrask.chla.data)<-c(colnames(Ortrask.sample.list),"substrate_trtmt","chla_ug_cm2")
write.csv(Ortrask.chla.data,"Ortrask.substrates.chlaData.csv")
...
```{r Load list of samples and abs spectra files, then calculate chl-a CHIRO SEDIMENT}

#Load list of samples from JULY 2019 ONLY
Chiro.sample.list<-read.csv("D:/UmeaResearch/2019 campaign/ChiroSed_chla_filesID.csv",header=T)

setwd("D:/UmeaResearch/2019 campaign/ChiroSedimentChla")
...
#Fixed variables
Vext<-40 #volume of extract in ml
t.area<-3.14159*(2.9/2)^2 # surface area of sediment sampled, based on inner diameter of falcom tube = 29mm (cm^2); note that we sampled to a depth of 1.5 cm 

#Vector to store chla calculations
Chiro.chla.ug.cm2<-NULL
for (i in 1:nrow(Chiro.sample.list)){
    Chiro.chla.ug.cm2[i]<-26.7*
    ((as.numeric(as.character(unacid[[which(names(unacid)==paste0(Chiro.sample.list[["filename"]][i],".csv"))]][which(as.numeric(as.character(unacid[[which(names(unacid)==paste0(Chiro.sample.list[["filename"]][i],".csv"))]][,1]))==665),10]))-as.numeric(as.character(unacid[[which(names(unacid)==paste0(Chiro.sample.list[["filename"]][i],".csv"))]][which(as.numeric(as.character(unacid[[which(names(unacid)==paste0(Chiro.sample.list[["filename"]][i],".csv"))]][,1]))==700),10])))
                    -(as.numeric(as.character(acid[[which(names(acid)==paste0(Chiro.sample.list[["filename"]][i],"HCl.csv"))]][which(as.numeric(as.character(acid[[which(names(acid)==paste0(Chiro.sample.list[["filename"]][i],"HCl.csv"))]][,1]))==664),10]))-as.numeric(as.character(acid[[which(names(acid)==paste0(Chiro.sample.list[["filename"]][i],"HCl.csv"))]][which(as.numeric(as.character(acid[[which(names(acid)==paste0(Chiro.sample.list[["filename"]][i],"HCl.csv"))]][,1]))==700),10]))))*Vext/t.area
}

ChiroSed.chla.data<-cbind(Chiro.sample.list,Chiro.chla.ug.cm2)
colnames(ChiroSed.chla.data)<-c(colnames(Chiro.sample.list),"chla_ug_cm2")
write.csv(ChiroSed.chla.data,"D:/UmeaResearch/2019 campaign/Chiro.BenthicSediment.chlaData.csv")
